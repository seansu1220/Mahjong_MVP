...
**修改內容**
-  新增 ，用冷色跟「已選取」的暖色區隔，並抬起 12px。
-  新增 ，
  依計數陣列標出指定張數（碰標 2 張、明槓標 3 張、吃標搭配的那 2 張）。
-  新增  事件（以  接 PointerEnter／PointerExit）。
-  在出現叫牌按鈕時，先把所有可叫動作會用到的牌一起標起來；
  滑鼠移到個別按鈕上時，縮小到只標那一組。
...
編輯器開著專案，改以 Unity 內附 Roslyn 搭配 Unity 參考組件編譯
Core 與 View 共 16 個檔案：**0 錯誤、0 警告**。Core 測試維持 **91 項全綠**。
